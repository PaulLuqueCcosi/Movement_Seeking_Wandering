...
class Mob(pg.sprite.Sprite):
    ...
    def wander_improved(self):
        ...
        target = future + vec(WANDER_RING_RADIUS, 0).rotate(uniform(0,
                                                                    360))
    ...
    def draw_vectors(self):
        scale = 25
        # vel
        pg.draw.line(screen, GREEN, self.pos, (self.pos + self.vel *
                                               scale), 5)
        # desired
        pg.draw.line(screen, RED, self.pos, (self.pos + self.desired *
                                             scale), 5)
        # target
        if WANDER_TYPE == 1:
            pg.draw.circle(screen, CYAN, (int(self.target.x),
                                          int(self.target.y)), 8)
        else:
